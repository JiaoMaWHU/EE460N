...
void MEM_stage() {
   ...
   if (PS.MEM_V) {
      if (Get_BR_OP(PS.MEM_CS)) {
         int n = (PS.MEM_IR>>11) - (PS.MEM_IR>>11<<1);
         int z = (PS.MEM_IR>>10) - (PS.MEM_IR>>10<<1);
         int p = (PS.MEM_IR>>9) - (PS.MEM_IR>>9<<1);
         int N1 = (PS.MEM_CC & 0x4) >> 2;
         int Z1 = (PS.MEM_CC & 0x2) >> 1;
         int P1 = PS.MEM_CC & 0x1;
         int ben = (n & N1) | (z & Z1) | (p & P1);
         /*printf("%d\n", ben);*/
         if (ben) mem_pcmux = 1;
      }
