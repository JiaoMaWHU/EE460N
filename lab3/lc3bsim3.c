...
int memoryOut;
int ADDERres;
int PCMUXres, MARMUXres, ADDR1MUXres, ADDR2MUXres, drNum, sr1, sr2;
...
void cycle_memory() {
   ...
   /* I think current latch ready bit should be set to 0 at initialization (can't
      modify the shell tho), but since we know the first state has no memory
      access, we can do this here, first setting READY to 0 at the beginning of
      every cycle, then evaluate and see if we need to assert R */
   ...
   if (GetMIO_EN(microcode)) {
      ...
      if (memenCycle < MEM_CYCLES)
         ...
         if (GetR_W(microcode) == 0){  /* read */
            int addr = CURRENT_LATCHES.MAR>>1<<1; /* pressumed no unaligned load, so this is okay */
...
void eval_bus_drivers() {
   ...
   /* MUX */
   if (GetDRMUX(microcode))
      drNum = 7;
   else
      drNum = (ir>>9) - (ir>>12<<3);

   if (GetSR1MUX(microcode))
...
void latch_datapath_values() {
   ...
   /* PCMUX     this one has to be here cuz it may use bus value*/
