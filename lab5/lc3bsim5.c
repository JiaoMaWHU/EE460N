...
enum CS_BITS {
    ...
    TRAPMUX,
    MDRMUX1, MDRMUX0,
    ATRSMUX3, ATRSMUX2, ATRSMUX1, ATRSMUX0,
    ATR,
    CONTROL_STORE_BITS
} CS_BITS;
...
int GetAT(int *x)            { return(x[AT]); }
int GetLD_SAVED_PSR(int *x)  { return(x[LD_SAVED_PSR]); }
int GetGATE_PTE_PA(int *x)   { return(x[GATE_PTE_PA]); }
int GetGATE_SAVED_PSR(int *x){ return(x[GATE_SAVED_PSR]); }
int GetPTEMUX(int *x)        { return(x[PTEMUX]); }
int GetILLEGALOPMUX(int *x)  { return(x[ILLEGALOPMUX]); }
int GetTRAPMUX(int *x)       { return(x[TRAPMUX]); }
int GetMDRMUX(int *x)        { return((x[MDRMUX1] << 1) + x[MDRMUX0]); }
int GetATRSMUX(int *x)       { return(x[ATRSMUX3] << 3) + (x[ATRSMUX2] << 2) + (x[ATRSMUX1] << 1) + x[ATRSMUX0]; }
int GetATR(int *x)           { return(x[ATR]); }
...
int TRAP; /* indicates whether current instruction is trap or not */
int ATRS; /* Address Translation Return State */
int SAVED_PSR; /* Temporary register to store the PSR */
...
int memoryOut;
int E2, E1, E0, E2temp;
int ADDERres, PCMUXres, MARMUXres, ADDR1MUXres, ADDR2MUXres, drNum, sr1, sr2,
      VECTORMUXres, SPMUXres, PRIVMUXres, TRAPMUXres, ATRSMUXres, MDRMUXres, PTEMUXres;
...
void eval_micro_sequencer() {
   ...
   /* logically this should be in the mux section, but the value of E is used in microsequencer,
      and that value of E is updated during the current state. In hardware it shouldnt make a
      difference if the cycle time is well-designed */
   if (GetILLEGALOPMUX(microcode)) {
      E2 = 0;
      E1 = 1;
      E0 = 0;
   }
   else {
      E2 = E2temp; /* E2temp is set under AT, thus the previous cycle */
      E1 = !CURRENT_LATCHES.TRAP & (CURRENT_LATCHES.PSR >> 15) & !((CURRENT_LATCHES.MDR >> 3) - (CURRENT_LATCHES.MDR>>4<<1));
      E0 = !((CURRENT_LATCHES.MDR >> 2) - (CURRENT_LATCHES.MDR>>3<<1)) | E1;
   }

   int temp = (GetATR(microcode)<<1) + GetIRD(microcode);
   if (temp == 1)
      NEXT_LATCHES.STATE_NUMBER = ir >> 12;
   else if (temp == 2)
      NEXT_LATCHES.STATE_NUMBER = CURRENT_LATCHES.ATRS;
   else {
      ...
      else if (GetCOND(microcode) == 6) /*E[2]*/
         NEXT_LATCHES.STATE_NUMBER = GetJ(microcode) | ((E2<<4) + E2);
      else  /* E[0] */
         NEXT_LATCHES.STATE_NUMBER = GetJ(microcode) | (E0<<3);
   }
...
void eval_bus_drivers() {
   ...
   /* MUX */
   if (GetTRAPMUX(microcode))
      TRAPMUXres = 1;
   else
      TRAPMUXres = 0;

   if (GetPTEMUX(microcode))
      PTEMUXres = (((CURRENT_LATCHES.MDR>>9)-(CURRENT_LATCHES.MDR>>14<<5))<<9) + (CURRENT_LATCHES.VA & 0x01FF);
   else
      PTEMUXres = CURRENT_LATCHES.MDR;

   switch (GetATRSMUX(microcode)) {
      case 0: {ATRSMUXres = 49; break;}
      case 1: {ATRSMUXres = 41; break;}
      case 2: {ATRSMUXres = 40; break;}
      case 3: {ATRSMUXres = 33; break;}
      case 4: {ATRSMUXres = 37; break;}
      case 5: {ATRSMUXres = 36; break;}
      case 6: {ATRSMUXres = 29; break;}
      case 7: {ATRSMUXres = 25; break;}
      case 8: {ATRSMUXres = 23; break;}
      case 9: {ATRSMUXres = 24; break;}
      case 10: {ATRSMUXres = 28; break;}
   }
   ...
   if (GetGATE_PC(microcode))
      ...
      if (GetDATA_SIZE(microcode)) { /* word */
         ...
         int MAR0 = CURRENT_LATCHES.MAR - (CURRENT_LATCHES.MAR>>1<<1);
         if (MAR0)
            busValue = signextend(PTEMUXres >> 8, 8, 16);
         else
            busValue = signextend(PTEMUXres - (PTEMUXres >> 8 << 8), 8, 16);
      }
   }
   ...
   else if (GetGATE_PTE_PA(microcode))
      busValue = CURRENT_LATCHES.PTBR + (CURRENT_LATCHES.MAR >> 9 << 1);

   else if (GetGATE_SAVED_PSR(microcode))
      busValue = CURRENT_LATCHES.SAVED_PSR;
...
void latch_datapath_values() {
   ...
   if (GetMDRMUX(microcode) == 1)
      MDRMUXres = BUS | 0x0001;
   else if (GetMDRMUX(microcode) == 2)
      MDRMUXres = BUS | 0x0003;
   else
      MDRMUXres = BUS;
   ...
   /* Load values */
   if (GetAT(microcode)) {
      E2temp = GetDATA_SIZE(microcode) & (BUS - (BUS>>1<<1));
      /* have to change E2temp here since otherwise it overwrites the value determined by MUX, which comes after this in microarchitecture*/
      NEXT_LATCHES.TRAP = TRAPMUXres;
      NEXT_LATCHES.ATRS = ATRSMUXres;
      NEXT_LATCHES.VA = BUS;
   }

   if (GetLD_SAVED_PSR(microcode))
      NEXT_LATCHES.SAVED_PSR = BUS;
   ...
   if (GetLD_MDR(microcode)) {
      if (GetMIO_EN(microcode))
         ...
         if (GetDATA_SIZE(microcode))  /* word */
            NEXT_LATCHES.MDR = MDRMUXres;
         else  /* byte */
            NEXT_LATCHES.MDR = ((MDRMUXres & 0x00FF) << 8) + (MDRMUXres & 0x00FF);
      }
   }
   ...
   if (GetLD_EXCV(microcode)) {
      int E = (E2<<2) + (E1<<1) + E0;
      if (E == 1) NEXT_LATCHES.EXCV = 0x02;
      else if (E == 2) NEXT_LATCHES.EXCV = 0x05;
      else if (E == 3) NEXT_LATCHES.EXCV = 0x04;
      else if (E >= 4) NEXT_LATCHES.EXCV = 0x03;
      /* E could be 0, which represents no exception; in code this can be omitted since when no exception vector won't be loaded
         In real MUX, either let output float or output any value, since it wont be used anyways*/
   }
